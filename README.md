...
pytest -q
python pipelines/orchestrator.py --dry-run   # ingest local; silver ainda lê Bronze
...
CI: [`.github/workflows/tests.yml`](.github/workflows/tests.yml) roda pytest em **cada pasta**.

`data/` de cada fase está no `.gitignore`. CSVs de origem da Fase 2 ficam em `fase2/raw_downloads/`.
